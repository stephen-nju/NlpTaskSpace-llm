...
def main():
    ...
    if finetune_args.block_size is None:
        ...
        if finetune_args.block_size > tokenizer.model_max_length:
            logger.warning(
                f"The block_size passed ({finetune_args.block_size}) is larger than the maximum length for the model"
                f"({tokenizer.model_max_length}). Using block_size={tokenizer.model_max_length}."
            )
        block_size = min(finetune_args.block_size, tokenizer.model_max_length)
    ...
    if training_args.do_train:
        ...
        if finetune_args.max_train_samples is not None:
    ...
    if training_args.do_eval:
        ...
        if finetune_args.max_eval_samples is not None:
