...
class FinetuneArguments:
    ...
    quantization_bit: Optional[str] = field(
        default=None, metadata={"help": "quantization bit when loading model"})
...
def main():
        ...
        for i in range(len(examples["context"])):
            if examples["context"][i] and examples["ner"][i]:
                ...
                b_ids = tokenizer.encode(
                    ...
                    truncation=True,
                    max_length=finetune_args.max_target_length)
                ...
                labels = (
                    ...
                    b_ids + [model.generation_config.eos_token_id])
                # 构建 batch padding
                pad_len = max_seq_length - len(input_ids)
                input_ids = input_ids + [model.generation_config.pad_token_id
                                         ] * pad_len
                labels = labels + [model.generation_config.pad_token_id
                                   ] * pad_len

                if finetune_args.ignore_pad_token_for_loss:
                    labels = [(l if l != model.generation_config.pad_token_id
                               else -100) for l in labels]
    ...
    def preprocessing_function_eval(examples):
        ...
        if finetune_args.ignore_pad_token_for_loss:
            labels["input_ids"] = [[
                (l if l != model.generation_config.pad_token_id else -100)
                for l in label
