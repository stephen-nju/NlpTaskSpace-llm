...
from transformers import (AutoConfig, AutoModel, AutoModelForCausalLM,
                          ...
                          BloomTokenizerFast, DataCollatorForSeq2Seq,
...
class ModelArguments:
    ...
    neft_alpha: Optional[float] = field(
        default=0, metadata={"help": "The alpha parameter to control the noise magnitude in NEFTune. value can be 5."}
    )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if attention_mask is not None:
            ...
            attn_output = flash_attn_func(query_states, key_states, value_states, 0.0, softmax_scale=None, causal=True)
...
def main():
    ...
    if training_args.do_train:
        ...
        with training_args.main_process_first(desc="Train dataset tokenization"):
            ...
            replaced_labels = [
                label if label != IGNORE_INDEX else tokenizer.pad_token_id for label in list(train_dataset[0]["labels"])
            ]
    ...
    if training_args.do_train:
        ...
        if trainer.is_world_process_zero():
            ...
            replaced_labels = [
                label if label != IGNORE_INDEX else tokenizer.pad_token_id for label in sample["labels"][0]
            ]
